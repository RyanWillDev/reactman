...
## Each branch in this repo is a working version of the app.
...
class App extends React.Component {
  ...
  render() {
    ...
    return <h1>Let's make a hangman game</h1>;
  }
}
```
...
```javascript
const myComponent = props => <h1>Let's make a hangman game</h1>;
```
...
class App extends Component {
  ...
  render() {
    return <Btn buttonText="Start Game" onClick={this.startGame} />;
  }
}
```
  ...
  render() {
    return (
      <div>
        ...
        />
        <Btn
          clickHandler={this.togglePhraseVisibility}
          buttonText={this.state.visible ? "Hide Phrase" : "Show Phrase"}
        />
      </div>
    );
  }
...
export default class Gameboard extends React.Component {
  ...
  render() {
    return <PhraseModal setPhrase={this.setPhrase} />;
  }
}
```
...
export default class HangmanGame extends React.Component {
  render() {
    return <h1>I am the HangmanGame</h1>;
  }
}
```
  ...
  render() {
    return (
      <ul>
        {this.props.phrase
          .split("")
          .map((letter, index) => <li key={index}>{letter}</li>)}
      </ul>
    );
  }
}
...
export const PhraseSlot = props => {
  ...
  return (
    <li className={className}>
      {props.charObj.guessed ? props.charObj.char : ""}
    </li>
  );
};
```
  ...
  render() {
    return (
      <div>
        ...
        >
          {this.state.phraseMap.map((obj, i) => (
            <PhraseSlot key={i} charObj={obj} />
          ))}
        </ul>
...
export default class HangmanGame extends React.Component {
  ...
  updateGame = event => {
    this.setState(
      PhraseUtils.diffPhraseMap(this.state, event.target.innerHTML)
    );
  };
    ...
    return (
      <div style={{ display: "flex" }}>
        <div>
          ...
          >
            {this.state.phraseMap.map((charObj, i) => (
              <PhraseSlot key={i} charObj={charObj} />
            ))}
          </ul>
...
There are other ways around this, but I didn't want to take you \*_too_ far down the rabbit hole on your first journey.
